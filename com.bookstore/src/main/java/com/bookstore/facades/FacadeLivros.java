...
/**
 * 
 * @author NPG (Grupo 2) Classe Façade usada para interfacear certas
 *         funcionalidades relativas a Livro. Esse padrão de projeto foi
 *         escolhido para ser implementado, pois o mesmo busca facilitar a
 *         utilização de métodos de classes distintas que se relacionam entre
 *         si, por parte das classes clientes.
 */
@Component
...
//		Livro livro = new Livro(isbn, titulo, descricao, preco, edicao, anoPublicacao, qntdEstoque);
	...
	/**
	 *  Método usado para recuperar uma lista de livros através de ids
	 * @param ids lista de ids para listar os livros
	 * @return retorna uma lista de livros
	 */
	public List<Livro> recuperarLivros(List<Long> ids) {

		return livroService.recuperarPorIds(ids);
	 ...
	 * 
	 * @param campoOrdenacao String que especifica a partir de qual atributo de
	 *                       Livro a página será ordenada.
	 * @param sortDirection  Direção da ordenação, que pode ser ascendente ou
	 *                       descendente.
	 * @param numeroPagina   Número da página que se deseja obter os registros
	 * @param inEstoque      Boolean que especifica se a busca deve ser realizada
	 *                       apenas entre os livros em estoque ou em geral.
	 * @param categorias     id das categorias para filtragem
	 * @param stringDeBusca  string de busca relaconada ao titulo do livro que se
	 *                       deseja buscar
	...
	/**
	 * Método retorna uma entidade com os atributos nulos, ou seja, vazios, para
	 * serem adicionados nos campos do fromulário
	 * 
	 ...
	 * @throws Exception lança excessão caso o id não corresponda a algum livro do
	 *                   banco
	 */
